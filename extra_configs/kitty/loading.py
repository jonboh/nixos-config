...
style_simple = (
    os.environ.get("KITTY_SCROLLBACK_NVIM_STYLE_SIMPLE", False).lower() == "true"
)
status_window_enabled = (
    os.environ.get("KITTY_SCROLLBACK_NVIM_STATUS_WINDOW_ENABLED", False).lower()
    == "true"
)
show_timer = os.environ.get("KITTY_SCROLLBACK_NVIM_SHOW_TIMER", False).lower() == "true"

timer_color = env_to_fg_color("KITTY_SCROLLBACK_NVIM_STATUS_WIN_NORMAL_HIGHLIGHT")
heart_color = env_to_fg_color("KITTY_SCROLLBACK_NVIM_STATUS_WIN_HEART_ICON_HIGHLIGHT")
spinner_color = env_to_fg_color(
    "KITTY_SCROLLBACK_NVIM_STATUS_WIN_SPINNER_ICON_HIGHLIGHT"
)
kitty_color = env_to_fg_color("KITTY_SCROLLBACK_NVIM_STATUS_WIN_KITTY_ICON_HIGHLIGHT")
nvim_color = env_to_fg_color("KITTY_SCROLLBACK_NVIM_STATUS_WIN_NVIM_ICON_HIGHLIGHT")
reset = "\x1b[0m"
...
while True and time_elapsed < close_threshold_seconds:
    if status_window_enabled:
        ...
        if style_simple:
            actual_timer = (
                time_progress
                if (show_timer and time_progress)
                or (time_elapsed and time_elapsed) > hide_timer_threshold_seconds
                else ""
            )
            ...
            line = ll.rjust(
                size.columns
                + (len(actual_timer_color + reset if actual_timer_color else ""))

...
with lib; let
  cfg = config.jonboh.services.kiwix-serve;
in {
  options = {
    jonboh.services.kiwix-serve = {
  ...
  config = mkIf cfg.enable {
    systemd.services.kiwix-serve = let
      maybeListenAddress = lib.optionals (cfg.listenAddress != null) ["--address" cfg.listenAddress];
      args = maybeListenAddress ++ ["--port" cfg.port] ++ cfg.zimPaths;
    in {
